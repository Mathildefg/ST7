...
GPR_mc(1).p = p;                               %set the GPR(1).p to have the data from p
GPR_mc(1).sysOut = sysOut;                     %set the GPR(1).sysOut to have the data from sysOut
GPR_mc(1).target = target;                     %set the GPR(1).target to have the data from target
GPR_mc(1).time = time;                         %set the GPR(1).time to have the data from time
...
i_gpr_mc = zeros(100,1);
j = 1;
for ii = 1:length(GPR_mc.time)-1
    if GPR_mc.time(ii+1)-GPR_mc.time(ii)>1
        A = GPR_mc.time(ii);
        i_gpr_mc(j) = find(GPR_mc.time==A);
        j=j+1;
    end
end
i_gpr_mc = nonzeros(i_gpr_mc);

if length(i_gpr_mc) == 31
    disp('Yes')
else 
    disp('No, there is a mistake')
end
i_gpr_mc = i_gpr_mc+1; i_gpr_mc  = [i_gpr_mc;length(GPR_mc.time)];
...
B = (Score_LR - Score_GPR_mc)/Score_LR;       
...
dt = 0.01;
GPR_mc(1).path.opt = zeros(length(i_gpr_mc),1);       %create an array of zeros with dim length(i_gpr) by 1 called GPR(1).path.opt
LR(1).path.opt = zeros(length(i_lr),1);         %create an array of zeros with dim length(i_lr) by 1 called LR(1).path.opt
GPR_mc(1).path.taken = zeros(length(i_gpr_mc),1);     %create an array of zeros with dim length(i_gpr) by 1 called GPR(1).path.taken
...
for jj = 1:32   %run from jj equal to 1 through 48.    Look into the equations below for Path optimising
    GPR_mc.path.opt(jj) = sqrt(GPR_mc.target(i_gpr_mc(jj)-1,1)^2+GPR_mc.target(i_gpr_mc(jj)-1,2)^2)-0.075; %0.075 is the radius of the target
    LR.path.opt(jj) = sqrt(LR.target(i_lr(jj)-1,1)^2+LR.target(i_lr(jj)-1,2)^2)-0.075;
    if jj == 1  %if jj equals to 1, do:
        gprOut_mc = [0,0;GPR_mc.sysOut(1:i_gpr_mc(jj)-1,1),GPR_mc.sysOut(1:i_gpr_mc(jj)-1,2)];     %GPR out is set to be concatenate of 0,0 and GPR.sysOut(1:i_gpr(jj)-1,1),GPR.sysOut(1:i_gpr(jj)-1,2)
        ...
        tmax_gpr_mc = length(gprOut_mc);          %returns the length of the longest array dimension in gprOut
        tmax_lr = length(lrOut);            %returns the length of the longest array dimension in lrOut
        t_gpr_mc = [1:dt:tmax_gpr_mc];            %t_gpr = [start:step:stop] -> start at 1, go 0.01 each step, end at tmax_gpr
        t_lr = [1:dt:tmax_lr];              %t_lr = [start:step:stop] -> start at 1, go 0.01 each step, end at tmax_lr
        gprOut_mc_new = interp1(gprOut_mc, t_gpr_mc, 'pchip');   %set gprOut_new to be 1D interpolation of gprOut and t_gpr with 
        ...
        lrOut_new = interp1(lrOut, t_lr, 'pchip');      %set lrOut_new to be 1D interpolation of lrOut and t_lr with 
                                                        %method pchip (Piecewise Cubic Hermite Interpolating Polynomial)
        count = 2;
        for kk = 3:length(gprOut_mc_new)           %for loop when kk equals 3 through the length of gprOut_new
        ...
        real_end_gpr_mc = length(gprOut_mc_new)-count+1;  %real_end_gpr equals the length of gprOut_new - (count+1)
    ...
    elseif jj == 32                 %else if jj is equal to 48 do:
        ...
        tmax_gpr_mc = length(gprOut_mc);  %set tmax_gpr to be the length of gprOut
...
% GPR
gpr_targets_mc = GPR_mc.target([1;i_gpr_mc(1:end-1)],:);
gpr_targets_check_mc = zeros(length(gpr_targets_mc),1);
for jj = 1:length(gpr_targets_mc)
    if abs(gpr_targets_mc(jj,1)) == abs(gpr_targets_mc(jj,2))
        gpr_targets_check_mc(jj) = 1;
    end
end
i_gpr_d_mc = find(gpr_targets_check_mc);          %find the indexes and values of the nonzero elements in gpr_targets_check
i_gpr_nd_mc = find(gpr_targets_check_mc==0);      %find the index equal to 0 in the gpr_targets_check
...
path_ratio_gpr_d_mc=(GPR_mc.path.taken(i_gpr_d_mc) - GPR_mc.path.opt(i_gpr_d_mc))./GPR_mc.path.opt(i_gpr_d_mc);
path_ratio_gpr_nd_mc=(GPR_mc.path.taken(i_gpr_nd_mc) - GPR_mc.path.opt(i_gpr_nd_mc))./GPR_mc.path.opt(i_gpr_nd_mc);
...
Performance(subject,2) = Score_GPR_mc;
Performance(subject,3) = B;
Performance(subject,4) = mean(path_ratio_lr);
Performance(subject,5) = mean(path_ratio_gpr_mc);
Performance(subject,6) = (Performance(subject,4)-Performance(subject,5))./Performance(subject,4);

%Performance_d = zeros(10,6);
Performance_d(subject,1) = mean(path_ratio_gpr_d_mc);
Performance_d(subject,2) = mean(path_ratio_gpr_nd_mc);
