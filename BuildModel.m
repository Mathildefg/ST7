...
hp = [gprMdl_dof2.Sigma,gprMdl_dof2.KernelInformation.KernelParameters'; ...
      gprMdl_dof3.Sigma,gprMdl_dof3.KernelInformation.KernelParameters'; ...
      gprMdl_dof5.Sigma,gprMdl_dof5.KernelInformation.KernelParameters'; ...
      gprMdl_dof6.Sigma,gprMdl_dof6.KernelInformation.KernelParameters'];      
        
% Make hyper parameters to build GPR model. Chosen based on tests.
hp_new = [hp(:,1)*(0.1/mean(hp(:,1))),hp(:,2)*(0.7/mean(hp(:,2))),hp(:,3)*(0.7/mean(hp(:,3)))];

% GPR with adjusted kernel parameters. This one is used for the experiment
gprMdl_dof2 = fitrgp(x1,y1,'Fitmethod','none','Sigma',0.01,'KernelParameters',[hp_new(1,2);hp_new(1,3)],'Basisfunction','none');
gprMdl_dof3 = fitrgp(x2,y2,'Fitmethod','none','Sigma',0.01,'KernelParameters',[hp_new(2,2);hp_new(2,3)],'Basisfunction','none');
gprMdl_dof5 = fitrgp(x3,y3,'Fitmethod','none','Sigma',0.01,'KernelParameters',[hp_new(3,2);hp_new(3,3)],'Basisfunction','none');
gprMdl_dof6 = fitrgp(x5,y5,'Fitmethod','none','Sigma',0.01,'KernelParameters',[hp_new(4,2);hp_new(4,3)],'Basisfunction','none');
